...
def test_triple_source(source):
  ...
  np.testing.assert_equal(source.test_set, np.array([[1, 2, 3]],
                                                    dtype=np.int64))
...
def test_data_loader(source, config):
  config = Config()
  data_loader = data.create_dataloader(source, config, False,
  ...
  np.testing.assert_equal(
      batch.numpy(),
      np.array([[0, 0, 1], [0, 1, 2], [1, 2, 3], [3, 1, 2], [2, 0, 3]],
               dtype=np.int64))
  np.testing.assert_equal(negatives.numpy()[0, :],
                          np.array([
                              [0, 0, 0],
                              [0, 2, 1],
                          ], dtype=np.int64))
...
def test_data_loader_with_label(source, config):
  config = Config()
  data_loader = data.create_dataloader(source, config, True,
                                       constants.DatasetType.TRAINING)
  sample_batched = next(iter(data_loader))
  batch, negatives, labels = sample_batched
  assert batch.shape == (5, 3)
  np.testing.assert_equal(
      ...
      np.array([[0, 0, 1], [0, 1, 2], [1, 2, 3], [3, 1, 2], [2, 0, 3]],
               dtype=np.int64))
  np.testing.assert_equal(
      batch.transpose(0, 1)[0].numpy(), np.array([0, 0, 1, 3, 2],
                                                 dtype=np.int64))
...
def test_dataloader_padding(config, source, cuda_device_2):
  ...
  np.testing.assert_equal(
      batch.numpy(),
      np.array(
          [[0, 0, 1], [0, 1, 2], [1, 2, 3], [3, 1, 2], [2, 0, 3], [0, 0, 1]],
          dtype=np.int64))
  np.testing.assert_equal(
      batch.transpose(0, 1)[0].numpy(),
      np.array([0, 0, 1, 3, 2, 0], dtype=np.int64))
