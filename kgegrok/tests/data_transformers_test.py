...
class DataTransformerTest(unittest.TestCase):
  ...
  def setUp(cls):
    ...
    cls.num_corrupts = 1
    cls.samples = (np.array([True, False], dtype=np.bool),
  ...
  def test_uniform_corruption_flag_generator(self):
    ...
    np.testing.assert_equal(
        transformers.CorruptionFlagGenerator(corruptor)(
            self.small_triple_list)[0],
        np.array([False, False], dtype=np.bool).reshape((-1,
                                                         self.num_corrupts)))
...
class DataTransformerCWASamplerTest(unittest.TestCase):
  ...
  def _build_corruptor_sampler(self):
    corruptor = self._build_corruptor()
    negative_sampler = kgedata.CWASampler(self.source.num_entity,
                                          self.source.num_relation, True)
    return corruptor, negative_sampler
