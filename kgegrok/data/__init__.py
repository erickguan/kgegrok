...
class TripleSource(object):
  ...
  def _collect_stats(self):
    ...
    max_nums = np.amax(
        np.stack([max_train, max_valid, max_test], axis=0), axis=0)
...
class TripleDataset(Dataset):
  ...
  def _build_batches(self, triples):
    ...
    if self._drop_last and len(
        self._data) > 1 and self._data[-1].shape != self._data[-2].shape:
      self._data.pop()
    elif self._pad_batch:
      num_pads = self._data[-1].shape[0] % max(MIN_NUM_PAD,
                                               kgegrok.utils.num_cuda_devices())
      while num_pads > 0:
...
def create_dataloader(triple_source,
  ...
  if dataset_type == constants.DatasetType.TRAINING:
    corruptor = kgedata.BernoulliCorruptor(
        triple_source.train_set, triple_source.num_relation,
        config.negative_entity, config.base_seed + SEED_OFFSET)
    ...
    dataset = TripleDataset(
        triple_set, batch_size=batch_size, transform=Compose(transforms))
